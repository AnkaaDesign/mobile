...
export const personalVacationsListConfig: ListConfig<Vacation> = {
  ...
  export: {
    ...
    columns: [
      ...
      { key: 'status', label: 'Status', path: 'status', format: (value: any): string => VACATION_STATUS_LABELS[value as keyof typeof VACATION_STATUS_LABELS] || value },
      { key: 'type', label: 'Tipo', path: 'type', format: (value: any): string => value ? VACATION_TYPE_LABELS[value as keyof typeof VACATION_TYPE_LABELS] : '-' },
      { key: 'isCollective', label: 'Coletivo', path: 'isCollective', format: (value: any): string => value ? 'Sim' : 'Não' },
