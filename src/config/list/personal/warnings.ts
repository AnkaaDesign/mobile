...
export const personalWarningsListConfig: ListConfig<Warning> = {
  ...
  export: {
    ...
    columns: [
      { key: 'severity', label: 'Severidade', path: 'severity', format: (value: any): string => WARNING_SEVERITY_LABELS[value as keyof typeof WARNING_SEVERITY_LABELS] || value },
      { key: 'category', label: 'Categoria', path: 'category', format: (value: any): string => value ? WARNING_CATEGORY_LABELS[value as keyof typeof WARNING_CATEGORY_LABELS] : '-' },
      ...
      { key: 'isActive', label: 'Ativo', path: 'isActive', format: (value: any): string => value ? 'Sim' : 'Não' },
