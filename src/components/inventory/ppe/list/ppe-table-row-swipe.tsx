...
const PpeTableRowSwipeComponent = ({ children, ppeId, ppeName, onEdit, onDelete, customActions = [], style, disabled = false }: PpeTableRowSwipeProps) => {
  ...
  const isThisRowActive = activeRowId === ppeId;
  ...
  useEffect(() => {
    return () => {
      ...
      if (activeRowId === ppeId) {
        setActiveRowId(null);
      }
    };
  }, [activeRowId, ppeId, setActiveRowId]);

  const handleDeletePress = useCallback(() => {
    Alert.alert("Confirmar exclusão", `Tem certeza que deseja excluir "${ppeName}"?`, [
      {
        text: "Cancelar",
        style: "cancel",
      ...
      },
      {
        ...
        onPress: () => {
          swipeableRef.current?.close();
          setTimeout(() => onDelete?.(ppeId), 300);
        },
      },
    ]);
  }, [ppeId, ppeName, onDelete]);
  ...
  const handleWillOpen = useCallback(
    (direction: "left" | "right") => {
      ...
      if (activeRowId && activeRowId !== ppeId) {
        closeActiveRow();
        closeOpenRow(); // Also close legacy rows
      }
    },
    [activeRowId, ppeId, closeActiveRow, closeOpenRow],
  );
  ...
  const handleOpen = useCallback(
    ...
    [ppeId, setActiveRowId, setOpenRow],
  );
  ...
  if (!children || (typeof children !== "object" && typeof children !== "string" && typeof children !== "number" && typeof children !== "function")) {
    console.warn("PpeTableRowSwipe: children prop is invalid or undefined:", typeof children);
