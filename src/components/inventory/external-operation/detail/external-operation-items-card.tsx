...
import type { ExternalOperationItem } from "@/types";
import { formatCurrency } from "@/utils";
import { useCanViewPrices } from "@/hooks";
import { EXTERNAL_OPERATION_TYPE, EXTERNAL_OPERATION_STATUS } from "@/constants";

interface ExternalOperationItemsCardProps {
  items: ExternalOperationItem[];
  withdrawalType?: EXTERNAL_OPERATION_TYPE;
  withdrawalStatus?: EXTERNAL_OPERATION_STATUS;
}

export function ExternalOperationItemsCard({ items, withdrawalType, withdrawalStatus }: ExternalOperationItemsCardProps) {
  const { colors } = useTheme();
  const canViewPrices = useCanViewPrices();

  const isReturnable = withdrawalType === EXTERNAL_OPERATION_TYPE.RETURNABLE;
  const isDelivered = withdrawalStatus === EXTERNAL_OPERATION_STATUS.DELIVERED;
  const isCharged = withdrawalStatus === EXTERNAL_OPERATION_STATUS.CHARGED;
  const isLiquidated = withdrawalStatus === EXTERNAL_OPERATION_STATUS.LIQUIDATED;
