// apps/mobile/src/components/inventory/external-operation/form/external-operation-form-utils.ts
...
import type { Item, ExternalOperation } from "@/types";
...
export interface ExternalOperationFormItem {
  id: string;
  name: string;
  uniCode?: string | null;
  quantity: number;
...
export interface ExternalOperationFormData {
...
export interface TotalCalculation {
  itemId: string;
  item: ExternalOperationFormItem;
  quantity: number;
  price: number;
  subtotal: number;
...
export interface ExternalOperationTotals {
  totalItems: number;
 ...
 */
export function getBestItemPrice(item: ExternalOperationFormItem, manualPrice?: number | null): number {
 ...
 */
export function calculateItemTotal(item: ExternalOperationFormItem, quantity: number, manualPrice?: number | null): TotalCalculation {
 ...
 */
export function calculateExternalOperationTotals(
  selectedItems: Map<string, ExternalOperationFormItem>,
  quantities: Record<string, number>,
  prices: Record<string, number>,
  type: EXTERNAL_OPERATION_TYPE,
): ExternalOperationTotals {
 ...
 */
export function transformFormDataForAPI(formData: ExternalOperationFormData): ExternalOperationCreateFormData {
  const items: Array<{
    itemId: string;
  ...
  formData.selectedItems.forEach((item, itemId) => {
    ...
    const price = formData.type !== EXTERNAL_OPERATION_TYPE.CHARGEABLE ? null : formData.prices[itemId] || getBestItemPrice(item);
 ...
 */
export function transformAPIDataToFormData(externalOperation: ExternalOperation, items: Item[]): Partial<ExternalOperationFormData> {
  const selectedItems = new Map<string, ExternalOperationFormItem>();
  const quantities: Record<string, number> = {};
  const prices: Record<string, number> = {};

  // Transform items if they exist
  if (externalOperation.items) {
    externalOperation.items.forEach((withdrawalItem) => {
 ...
 */
export function cloneFormData(formData: ExternalOperationFormData): ExternalOperationFormData {
  return {
 ...
 */
export function validateItemSelection(selectedItems: Map<string, ExternalOperationFormItem>, quantities: Record<string, number>): ValidationError[] {
 ...
 */
export function validateItemPrices(selectedItems: Map<string, ExternalOperationFormItem>, prices: Record<string, number>, type: EXTERNAL_OPERATION_TYPE): ValidationError[] {
  const errors: ValidationError[] = [];
 ...
 */
export function validateExternalOperationForm(
  formData: ExternalOperationFormData,
  options: {
  ...
  const totals = calculateExternalOperationTotals(formData.selectedItems, formData.quantities, formData.prices, formData.type);

  // Validate total value if needed
  if (formData.type === EXTERNAL_OPERATION_TYPE.CHARGEABLE && options.maxTotal && totals.grandTotal > options.maxTotal) {
    errors.push({
      field: "total",
  ...
  // Add warnings
  if (formData.type === EXTERNAL_OPERATION_TYPE.CHARGEABLE && totals.hasItemsWithoutPrice) {
    warnings.push({
      field: "prices",
...
/**
 * Convert Item to ExternalOperationFormItem
 */
export function convertItemToFormItem(item: Item, quantity: number = 1, price?: number | null): ExternalOperationFormItem {
  return {
    id: item.id,
    name: item.name,
    uniCode: item.uniCode,
 ...
 */
export function generateSelectionSummary(selectedItems: Map<string, ExternalOperationFormItem>, quantities: Record<string, number>, type: EXTERNAL_OPERATION_TYPE): string {
  const totalItems = selectedItems.size;
  ...
  const returnText = type === EXTERNAL_OPERATION_TYPE.RETURNABLE ? " (para devolução)" : type === EXTERNAL_OPERATION_TYPE.COMPLIMENTARY ? " (cortesia)" : "";
...
export const externalOperationFormUtils = {
