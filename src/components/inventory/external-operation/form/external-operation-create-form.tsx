...
export function ExternalOperationCreateForm() {
  const { colors } = useTheme();
  const nav = useNav();
  const goBack = () =>
    nav.goBack({
      fallback: mobileRoute(routes.inventory.externalOperations.root),
    });
  ...
  const {
    ...
    storageKey: "@external_operation_create_form",
    defaultQuantity: 1,
    defaultPrice: 0,
    preserveQuantitiesOnDeselect: false,
    validateOnStageChange: true,
    requireCustomerOnChargeable: true,
    autoSave: true,
  });
  ...
  const handleNext = useCallback(() => {
    ...
    if (!success && formTouched) {
      ...
      if (stage === 1 && (validation.errors.customerId || validation.errors.withdrawerName)) {
        RNAlert.alert("Erro", validation.errors.customerId || validation.errors.withdrawerName!);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        await resetForm();
        nav.replace(mobileRoute(routes.inventory.externalOperations.details(result.data.id)));
      }
    } catch (error) {
  ...
  }, [stage, withdrawerName, customerId, type, notes, receiptFiles, nfeFiles]);
    ...
    <ScrollView style={styles.stageContainer} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      <Card style={styles.card}>
        ...
        <View style={styles.content}>
          {/* Customer — required for CHARGEABLE (billing target) */}
          <View style={styles.formField}>
            <FormLabel
              required={type === EXTERNAL_OPERATION_TYPE.CHARGEABLE}
              asteriskColor="red"
            >
              Cliente
            </FormLabel>
            ...
            />
            {formTouched && validation.errors.customerId && (
              <Text style={styles.errorText}>{validation.errors.customerId}</Text>
            )}
          </View>
