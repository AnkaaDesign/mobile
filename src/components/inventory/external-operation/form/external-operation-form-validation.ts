...
export const stage1ValidationSchema = z.object({
  withdrawerId: z.string().min(1, "Usuário responsável deve ser selecionado"),
  withdrawerName: z.string().min(2, "Nome do responsável deve ter pelo menos 2 caracteres"),
  type: z.nativeEnum(EXTERNAL_OPERATION_TYPE),
...
export function validateStage1(
  ...
  if (!data.withdrawerId && !data.withdrawerName) {
    errors.push({
      field: "withdrawerId",
      message: "Usuário responsável deve ser selecionado",
      type: "required",
    });
  }
  ...
  if (context.selectedUser) {
    ...
    if (context.selectedUser.sector?.privileges === "EXTERNAL") {
      errors.push({
        field: "withdrawerId",
        message: "Usuários externos não podem fazer operações diretas",
        type: "business",
      });
    }
  }
...
export function validateCompleteForm(
  ...
  const totalValue = data.type === EXTERNAL_OPERATION_TYPE.CHARGEABLE
  ...
  if (data.type === EXTERNAL_OPERATION_TYPE.CHARGEABLE && totalValue > 10000) {
    warnings.push({
      field: "total",
  ...
  return {
    ...
    errors,
    warnings,
    summary: errors.length > 0 ? `${errors.length} erro(s) impedem a criação da operação` : `Formulário válido - ${data.items.length} item(s), total: R$ ${totalValue.toFixed(2)}`,
  };
}
...
export function validateItemPrice(item: Item, price: number | null | undefined, type: EXTERNAL_OPERATION_TYPE): ValidationError[] {
  const errors: ValidationError[] = [];
...
export function validateBusinessRules(
  ...
  if (data.items.length > 50) {
    errors.push({
      field: "items",
      message: "Máximo de 50 itens por operação",
      type: "business",
    });
  }
...
export const externalOperationFormValidation = {
  validateStage1,
  validateStage2,
...
export default externalOperationFormValidation;
