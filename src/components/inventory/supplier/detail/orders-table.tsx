...
export function OrdersTable({ supplier, maxHeight = 500 }: OrdersTableProps) {
  ...
  const [visibleColumnKeys, setVisibleColumnKeys] = useState<string[]>(() => {
    return ["description", "status", "createdAt"];
  });
  ...
  // Fetch orders for this specific supplier with infinite scroll
  const {
    items: orders,
    isLoading,
    error,
  ...
  // Filter orders based on search (client-side for already loaded orders)
            ...
            <View style={StyleSheet.flatten([styles.emptyState, { backgroundColor: colors.muted + "20" }])}>
              ...
              <ThemedText style={StyleSheet.flatten([styles.emptyText, { color: colors.mutedForeground }])}>
                {searchQuery
                  ...
                  : "Nenhum pedido associado a este fornecedor."}
              </ThemedText>
            </View>
          ) : (
