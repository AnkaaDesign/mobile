...
interface SupplierTableProps {
  ...
  onSelectionChange?: (selectedSuppliers: Set<string>) => void;
...
export const createColumnDefinitions = (): TableColumn[] => [
  ...
  },
  {
    ...
    accessor: (supplier: Supplier) => {
      ...
      const phonesText = supplier.phones
        .slice(0, 2)
      ...
      return (
        <ThemedText style={StyleSheet.flatten([styles.cellText, styles.monoText])} numberOfLines={1}>
          {phonesText}
          {supplier.phones.length > 2 ? ` +${supplier.phones.length - 2}` : ""}
        </ThemedText>
      );
    },
  },
  {
    ...
    accessor: (supplier: Supplier) => {
      ...
      const addressParts = [supplier.address, supplier.addressNumber, supplier.neighborhood].filter(Boolean);
      return (
      ...
      <View style={styles.centerAlign}>
        <Badge variant="outline" size="sm">
          <ThemedText style={{ fontSize: fontSize.xs, fontFamily: 'monospace' }}>
            {(supplier as any)._count?.items || supplier.items?.length || 0}
          </ThemedText>
        </Badge>
      </View>
    ),
  },
  {
    ...
    accessor: (supplier: Supplier) => (
      <View style={styles.centerAlign}>
        <Badge variant="outline" size="sm">
          <ThemedText style={{ fontSize: fontSize.xs, fontFamily: 'monospace' }}>
            {(supplier as any)._count?.orders || supplier.orders?.length || 0}
          </ThemedText>
        </Badge>
      </View>
    ),
...
export const SupplierTable = React.memo<SupplierTableProps>(
  ({
    suppliers,
    onSupplierPress,
    onSupplierEdit,
    ...
    const handleSelectAll = useCallback(() => {
      ...
      const allSelected = suppliers.every((supplier) => selectedSuppliers.has(supplier.id));
      if (allSelected) {
        onSelectionChange(new Set());
      } else {
        onSelectionChange(new Set(suppliers.map((supplier) => supplier.id)));
      }
    }, [suppliers, selectedSuppliers, onSelectionChange]);
    ...
    const renderColumnValue = useCallback((supplier: Supplier, column: TableColumn) => {
      return column.accessor(supplier);
    }, []);
    ...
    const renderHeader = useCallback(
            ...
            <View style={StyleSheet.flatten([styles.headerRow, { width: tableWidth }])}>
              {showSelection && (
                <View style={StyleSheet.flatten([styles.headerCell, styles.checkboxCell])}>
                  <Checkbox checked={suppliers.length > 0 && suppliers.every((supplier) => selectedSuppliers.has(supplier.id))} onCheckedChange={handleSelectAll} disabled={suppliers.length === 0} />
                </View>
              )}
      ...
      [colors, isDark, tableWidth, displayColumns, showSelection, selectedSuppliers, suppliers.length, sortConfigs, handleSelectAll, handleSort],
    );

    // Row component
    const renderRow = useCallback(
      ({ item, index }: { item: Supplier; index: number }) => {
        const isSelected = selectedSuppliers.has(item.id);
                  ...
                  >
                    {showSelection && (
                      <View style={StyleSheet.flatten([styles.cell, styles.checkboxCell])}>
                        <Checkbox checked={isSelected} onCheckedChange={() => handleSelectSupplier(item.id)} />
                      </View>
                    )}
                    {displayColumns.map((column) => (
                      <View
    ...
    const renderEmpty = useCallback(
      () => (
        <View style={styles.emptyContainer}>
          ...
          <ThemedText style={styles.emptyTitle}>Nenhum fornecedor encontrado</ThemedText>
          <ThemedText style={styles.emptySubtitle}>Tente ajustar os filtros ou adicionar novos fornecedores</ThemedText>
        </View>
      ),
    ...
    if (loading) {
      return (
        <View style={styles.loadingContainer}>
          ...
          <ThemedText style={styles.loadingText}>Carregando fornecedores...</ThemedText>
        </View>
      );
    }
