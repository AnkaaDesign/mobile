...
interface SupplierColumnVisibilityDrawerProps {
...
export function SupplierColumnVisibilityDrawer({
  columns,
  visibleColumns,
  onVisibilityChange,
  open,
  onOpenChange
}: SupplierColumnVisibilityDrawerProps) {
  ...
  const renderItem = useCallback(({ item }: { item: SupplierColumn }) => (
    <ColumnListItem
      column={item}
  ...
  const keyExtractor = useCallback((item: SupplierColumn) => item.key, []);
