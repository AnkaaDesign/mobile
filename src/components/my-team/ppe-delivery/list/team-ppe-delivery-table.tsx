...
interface TeamPpeDeliveryTableProps {
  ...
  onDeliveryEdit?: (deliveryId: string) => void;
...
export const createColumnDefinitions = (): TableColumn[] => [
  ...
  },
  {
    ...
    accessor: (delivery: PpeDelivery) => (
      <ThemedText style={styles.cellText} numberOfLines={1}>
        {delivery.scheduledDate ? formatDate(new Date(delivery.scheduledDate)) : "-"}
      </ThemedText>
    ),
  },
  {
  ...
  },
  {
    ...
    accessor: (delivery: PpeDelivery) => (
      <ThemedText style={styles.cellText} numberOfLines={1}>
        {delivery.createdAt ? formatDate(new Date(delivery.createdAt)) : "-"}
      </ThemedText>
    ),
  },
];
...
export const TeamPpeDeliveryTable = React.memo<TeamPpeDeliveryTableProps>(
  ({
    ...
    sortConfigs = [],
    onSort,
    visibleColumnKeys = ["userName", "itemName", "quantity", "status", "deliveryDate"],
    ...
    const renderColumnValue = useCallback((delivery: PpeDelivery, column: TableColumn) => {
      return column.accessor(delivery);
    }, []);
    ...
    const renderRow = useCallback(
      ({ item, index }: { item: PpeDelivery; index: number }) => {
        const isEven = index % 2 === 0;

        if (enableSwipeActions && (onDeliveryEdit || onDeliveryDelete)) {
          return (
            <TeamPpeDeliveryTableRowSwipe
              key={item.id}
              deliveryId={item.id}
              deliveryName={`${item.user?.name} - ${item.item?.name}`}
    ...
    const renderEmpty = useCallback(
      () => (
        <View style={styles.emptyContainer}>
          ...
          <ThemedText style={styles.emptySubtitle}>Não há entregas de EPIs registradas para a equipe</ThemedText>
        </View>
      ),
      [],
    );
