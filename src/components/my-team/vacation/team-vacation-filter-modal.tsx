...
interface TeamVacationFilterModalProps {
  ...
  teamMemberIds: string[]; // IDs of team members to filter by
}

export const TeamVacationFilterModal = ({ visible, onClose, onApply, currentFilters, teamMemberIds }: TeamVacationFilterModalProps) => {
  ...
  const handleShowConflictsToggle = useCallback((checked: boolean) => {
    setFilters((prev) => ({
      ...prev,
  ...
  // Toggle show current vacations only
  const handleShowCurrentToggle = useCallback((checked: boolean) => {
    setFilters((prev) => ({
      ...prev,
      showCurrentOnly: checked,
    }));
  }, []);
  ...
  const handleApply = useCallback(() => {
    ...
    onApply(filtersWithTeam);
  }, [filters, onApply, teamMemberIds]);
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if ((filters as any).showConflictsOnly) count++;
    if ((filters as any).showCurrentOnly) count++;
    return count;
  }, [filters]);
      ...
      <ThemedView style={styles.container}>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            <ThemedText style={styles.sectionTitle}>Filtros Rápidos</ThemedText>
            <View style={styles.sectionContent}>
              <TouchableOpacity
                style={styles.checkboxRow}
                onPress={() => handleShowCurrentToggle(!(filters as any).showCurrentOnly)}
              >
                <Checkbox
                  checked={(filters as any).showCurrentOnly || false}
                  onCheckedChange={(checked) => handleShowCurrentToggle(checked as boolean)}
                />
                <ThemedText style={styles.checkboxLabel}>Apenas férias em andamento</ThemedText>
              </TouchableOpacity>

              <TouchableOpacity
                style={styles.checkboxRow}
                onPress={() => handleShowConflictsToggle(!(filters as any).showConflictsOnly)}
              >
                <Checkbox
                  checked={(filters as any).showConflictsOnly || false}
                  onCheckedChange={(checked) => handleShowConflictsToggle(checked as boolean)}
                />
                <ThemedText style={styles.checkboxLabel}>Apenas conflitos de férias</ThemedText>
              </TouchableOpacity>
            </View>
          </View>
