...
export function TeamCommissionStatsCard({ tasks }: TeamCommissionStatsCardProps) {
  ...
  const stats = useMemo(() => {
    ...
    // Calculate total earnings (tasks with quote)
    const totalEarned = fullCommissionTasks.reduce((sum, task) => sum + (task.quote?.total || 0), 0);
    const partialEarned = partialCommissionTasks.reduce((sum, task) => sum + (task.quote?.total || 0) * 0.5, 0); // Assuming 50% for partial
