  ...
  ({ value, onChange, onBlur, label, error, helperText, containerStyle, inputStyle, placeholder = "(00) 00000-0000", ...props }, ref) => {
    ...
    const handleChangeText = useCallback(
      (text: string) => {
        ...
        // Handle the broader type by converting to string or null
        onChange?.(numbers.length > 0 ? numbers : null);
      },
      [onChange]
    );

    useEffect(() => {
      // Handle value that could be string, number, or null
      const stringValue = value != null ? String(value) : "";
      const cleanValue = extractNumbers(stringValue);
