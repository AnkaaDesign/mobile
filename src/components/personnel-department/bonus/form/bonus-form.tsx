...
export function BonusForm({ mode, bonus, onSuccess, onCancel }: BonusFormProps) {
  ...
  const { data: users } = useUsers({
    // Bonus eligibility = CLT confirmado (ACTIVE). Mirrors the api isBonifiable
    // predicate (employeeType === CLT && status === ACTIVE).
    where: {
  ...
  const handleSubmit = async (data: BonusCreateFormData | BonusUpdateFormData) => {
    try {
      if (mode === "create") {
        ...
        onSuccess?.();
        if (newId) {
          nav.replace(mobileRoute(routes.personnelDepartment.bonifications.details(newId)));
        } else {
          goBack();
        }
      } else if (bonus) {
        ...
        onSuccess?.();
        nav.replace(mobileRoute(routes.personnelDepartment.bonifications.details(bonus.id)));
      }
    } catch {
