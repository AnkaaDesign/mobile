  ...
  const handleSubmit = async (data: LeaveCreateFormData | LeaveUpdateFormData) => {
    try {
      if (mode === "create") {
        ...
        onSuccess?.();
        if (newId) {
          nav.replace(mobileRoute(routes.personnelDepartment.occupationalHealth.leaves.details(newId)));
        } else {
          goBack();
        }
      } else if (leave) {
        ...
        onSuccess?.();
        nav.replace(mobileRoute(routes.personnelDepartment.occupationalHealth.leaves.details(leave.id)));
      }
    } catch {
