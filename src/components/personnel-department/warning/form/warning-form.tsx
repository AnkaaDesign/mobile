  ...
  const handleSubmit = async (data: WarningCreateFormData | WarningUpdateFormData) => {
    try {
      if (mode === "create") {
        ...
        onSuccess?.();
        if (newId) {
          nav.replace(mobileRoute(routes.personnelDepartment.warnings.details(newId)));
        } else {
          goBack();
        }
      } else if (warning) {
        ...
        onSuccess?.();
        nav.replace(mobileRoute(routes.personnelDepartment.warnings.details(warning.id)));
      }
    } catch {
