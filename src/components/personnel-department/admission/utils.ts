// Shared helpers for the Admission (Admissões) module (mobile).
// Mirrors web src/components/personnel-department/admission/utils.ts and the
...
// Sempre obrigatórios.
export const ADMISSION_HARD_REQUIRED_DOC_TYPES: ADMISSION_DOCUMENT_TYPE[] = [
  ADMISSION_DOCUMENT_TYPE.CPF,
  ADMISSION_DOCUMENT_TYPE.CTPS,
];
// Obrigatório "um destes": RG OU CNH.
export const ADMISSION_EITHER_REQUIRED_DOC_TYPES: ADMISSION_DOCUMENT_TYPE[] = [
  ADMISSION_DOCUMENT_TYPE.RG,
 ...
 */
export function hasBlockingRequiredDocs(
  admission: Pick<Admission, "status"> & { documents?: AdmissionDocument[] },
): boolean {
  if (admission.status !== ADMISSION_STATUS.DOCS_PENDING) return false;
  const docs = admission.documents || [];

  const hardPending = docs.some(
    (doc) => ADMISSION_HARD_REQUIRED_DOC_TYPES.includes(doc.type) && doc.status === ADMISSION_DOCUMENT_STATUS.PENDING,
  );
