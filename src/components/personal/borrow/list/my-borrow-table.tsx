...
export const createColumnDefinitions = (): TableColumn[] => [
  ...
  },
  {
    ...
    width: 0,
    accessor: (borrow: Borrow) => {
      const label = BORROW_STATUS_LABELS[borrow.status as keyof typeof BORROW_STATUS_LABELS];
      return (
        <Badge variant={getStatusBadgeVariant(borrow.status)} size="sm">
...
export const MyBorrowTable = React.memo<MyBorrowTableProps>(
  ({
    borrows,
    onBorrowPress,
    onBorrowEdit,
    ...
    sortConfigs = [],
    onSort,
    visibleColumnKeys = ["item", "status", "createdAt"],
...
MyBorrowTable.displayName = "MyBorrowTable";
