...
// Column interface for PaintType
...
export function PaintTypeColumnVisibilityDrawer({
  ...
  const renderItem = useCallback(({ item }: { item: PaintTypeColumn }) => (
    <ColumnListItem
      column={item}
  ...
  const keyExtractor = useCallback((item: PaintTypeColumn) => item.key, []);
