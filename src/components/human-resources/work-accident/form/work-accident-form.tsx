  ...
  const handleSubmit = async (data: WorkAccidentReportCreateFormData | WorkAccidentReportUpdateFormData) => {
    try {
      ...
      if (mode === "create") {
        ...
        onSuccess?.();
        if (newId) {
          nav.replace(mobileRoute(routes.humanResources.occupationalHealth.workAccidents.details(newId)));
        } else {
          goBack();
        }
        ...
        onSuccess?.();
        nav.replace(mobileRoute(routes.humanResources.occupationalHealth.workAccidents.details(workAccident.id)));
      }
    } catch {
