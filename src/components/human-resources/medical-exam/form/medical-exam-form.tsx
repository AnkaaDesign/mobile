  ...
  const handleSubmit = async (data: MedicalExamCreateFormData | MedicalExamUpdateFormData) => {
    ...
    try {
      if (mode === "create") {
        ...
        onSuccess?.();
        if (newId) {
          nav.replace(mobileRoute(routes.humanResources.occupationalHealth.medicalExams.details(newId)));
        } else {
          goBack();
        }
      } else if (medicalExam) {
        ...
        onSuccess?.();
        nav.replace(mobileRoute(routes.humanResources.occupationalHealth.medicalExams.details(medicalExam.id)));
      }
    } catch {
