...
export function getDefaultVisibleColumns(): Set<string> {
  return new Set(
    PAYROLL_COLUMNS.filter((col) => col.defaultVisible).map((col) => col.key)
  );
}

interface PayrollColumnVisibilityDrawerProps {
...
export function PayrollColumnVisibilityDrawer({
  visibleColumns,
  onVisibilityChange,
  open,
  onOpenChange,
}: PayrollColumnVisibilityDrawerProps) {
  ...
  const filteredColumns = useMemo(() => {
    if (!searchQuery.trim()) return PAYROLL_COLUMNS;
    const query = searchQuery.toLowerCase();
    return PAYROLL_COLUMNS.filter((col) => col.header.toLowerCase().includes(query));
  }, [searchQuery]);
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.actionsWrapper}>
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <ThemedText style={styles.actionBtnText}>Todas</ThemedText>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.actionBtn, { backgroundColor: colors.card, borderColor: colors.border }]}
            onPress={() => {
