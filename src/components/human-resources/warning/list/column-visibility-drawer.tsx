...
// Function to get default visible columns for warnings
...
export function ColumnVisibilityDrawer({
  ...
  const renderItem = useCallback(({ item }: { item: WarningColumn }) => (
    <ColumnListItem
      column={item}
  ...
  const keyExtractor = useCallback((item: WarningColumn) => item.key, []);
