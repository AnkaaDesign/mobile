...
interface UserCreatedTasksTableProps {
  user: User;
  maxHeight?: number;
}
...
export function UserCreatedTasksTable({ user, maxHeight = 500 }: UserCreatedTasksTableProps) {
  ...
  // Use name, customer, and user columns for created tasks view
  const [visibleColumnKeys, setVisibleColumnKeys] = useState<string[]>(() => {
    return ["name", "customer", "user"];
  });
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((task: any) =>
      ...
      task.user?.name?.toLowerCase().includes(lowerSearch)
    );
