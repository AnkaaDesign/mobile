...
interface SectorFilterDrawerV2Props {
  visible: boolean;
  onClose: () => void;
...
type SectionKey = "privileges" | "status";
...
export function SectorFilterDrawerV2({ visible, onClose, onApply, currentFilters }: SectorFilterDrawerV2Props) {
  const { colors } = useTheme();
  const insets = useSafeAreaInsets();
  const [filters, setFilters] = useState<FilterState>(currentFilters || {});
  const [expandedSections, setExpandedSections] = useState<Set<SectionKey>>(
    new Set(["privileges", "status"])
  );
  ...
  const activeFilterCount = useMemo(() => {
    let count = 0;
    if (filters.privilege) count++;
    if (filters.hasUsers !== undefined) count++;
    return count;
  }, [filters]);

  const handlePrivilegeToggle = useCallback((privilege: SECTOR_PRIVILEGES) => {
    setFilters((prev) => ({
      ...prev,
      privilege: prev.privilege === privilege ? undefined : privilege,
    }));
  }, []);

  const handleHasUsersToggle = useCallback((value: boolean) => {
    setFilters((prev) => ({
      ...prev,
      hasUsers: value || undefined,
    }));
  }, []);
      ...
      component: (
        <View style={styles.sectionContent}>
          {Object.values(SECTOR_PRIVILEGES).map((privilege) => {
            const privilegeLabel = SECTOR_PRIVILEGES_LABELS[privilege];
            const isSelected = filters.privilege === privilege;

            return (
              <View key={privilege} style={styles.row}>
                <ThemedText style={styles.label}>
                  {privilegeLabel}
                </ThemedText>
                <RNSwitch
                  value={isSelected}
                  onValueChange={() => handlePrivilegeToggle(privilege)}
      ...
      component: (
        <View style={styles.sectionContent}>
          <View style={styles.row}>
            <Label style={styles.label}>Somente com funcionários</Label>
            <RNSwitch
              value={!!filters.hasUsers}
              onValueChange={handleHasUsersToggle}
              trackColor={{ false: colors.muted, true: colors.primary }}
              thumbColor={!!filters.hasUsers ? colors.primaryForeground : "#f4f3f4"}
              ios_backgroundColor={colors.muted}
            />
          </View>
        </View>
  ...
  ], [filters, colors, handlePrivilegeToggle, handleHasUsersToggle]);
