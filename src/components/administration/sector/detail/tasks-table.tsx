...
export function TasksTable({ sector, maxHeight = 500 }: TasksTableProps) {
  const { colors } = useTheme();

  // Column panel state
  const [isColumnPanelOpen, setIsColumnPanelOpen] = useState(false);

  // Use only name and status columns for sector detail view
  ...
  // Fetch tasks for this specific sector with infinite scroll
  const {
    items: tasks,
    isLoading,
    error,
  ...
  // Filter tasks (client-side for already loaded tasks)
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((task) => {
      return (
        ...
        task.customer?.fantasyName?.toLowerCase().includes(lowerSearch) ||
  ...
  return (
    <>
      <DetailCard title={`Tarefas do Setor ${tasks.length > 0 ? `(${tasks.length}${totalCount ? `/${totalCount}` : ""})` : ""}`} icon="clipboard-list">
