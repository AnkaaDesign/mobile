...
const toggleTextVariants = cva("text-sm native:text-base text-neutral-800 font-medium", {
  variants: {
    variant: {
      default: "",
...
>(({ className, variant, size, ...props }, ref) => (
  <TextClassContext.Provider value={cn(toggleTextVariants({ variant, size }), props.pressed ? "text-accent-foreground" : "web:group-hover:text-muted-foreground", className)}>
    <TogglePrimitive.Root
      ref={ref}
      className={cn(toggleVariants({ variant, size }), props.disabled && "web:pointer-events-none opacity-50", props.pressed && "bg-accent", className)}
      {...props}
    />
...
function ToggleIcon({ className, icon: Icon, ...props }: ToggleIconProps) {
