...
interface TruckTableRowSwipeProps {
...
const TruckTableRowSwipeComponent = ({ children, truckId, truckPlate, onEdit, onDelete, customActions = [], style, disabled = false }: TruckTableRowSwipeProps) => {
  ...
  const isThisRowActive = activeRowId === truckId;
  ...
  useEffect(() => {
    return () => {
      ...
      if (activeRowId === truckId) {
        setActiveRowId(null);
      }
    };
  }, [activeRowId, truckId, setActiveRowId]);

  const handleDeletePress = useCallback(() => {
    Alert.alert("Confirmar exclusão", `Tem certeza que deseja excluir o caminhão "${truckPlate}"?`, [
      {
        text: "Cancelar",
        style: "cancel",
      ...
      },
      {
        ...
        onPress: () => {
          swipeableRef.current?.close();
          setTimeout(() => onDelete?.(truckId), 300);
        },
      },
    ]);
  }, [truckId, truckPlate, onDelete]);
  ...
  const handleWillOpen = useCallback(
    (_direction: "left" | "right") => {
      ...
      if (activeRowId && activeRowId !== truckId) {
        closeActiveRow();
        closeOpenRow(); // Also close legacy rows
      }
    },
    [activeRowId, truckId, closeActiveRow, closeOpenRow],
  );

  const handleOpen = useCallback(
    (_direction: "left" | "right", swipeable: Swipeable) => {
      setActiveRowId(truckId);
    ...
    [truckId, setActiveRowId, setOpenRow],
  );
  ...
  if (!children || (typeof children !== "object" && typeof children !== "string" && typeof children !== "number" && typeof children !== "function")) {
    console.warn("TruckTableRowSwipe: children prop is invalid or undefined:", typeof children);
...
TruckTableRowSwipeComponent.displayName = "TruckTableRowSwipe";

export const TruckTableRowSwipe = React.memo(TruckTableRowSwipeComponent);
