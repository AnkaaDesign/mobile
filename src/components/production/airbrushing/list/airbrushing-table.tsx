...
interface AirbrushingTableProps {
  ...
  onAirbrushingEdit?: (airbrushingId: string) => void;
  onAirbrushingDelete?: (airbrushingId: string) => void;
  ...
  selectedAirbrushings?: Set<string>;
  onSelectionChange?: (selectedAirbrushings: Set<string>) => void;
...
export const createColumnDefinitions = (): TableColumn[] => [
  {
    ...
    sortable: true,
    width: 0,
    accessor: (airbrushing: Airbrushing) => {
      const task = (airbrushing as any).task;
      if (!task) {
        return (
  ...
  },
  {
    ...
    width: 0,
    accessor: (airbrushing: Airbrushing) => {
      const variant = getBadgeVariantFromStatus(airbrushing.status, "AIRBRUSHING_STATUS");
      return (
  ...
  },
  {
    ...
    accessor: (airbrushing: Airbrushing) => {
      ...
      return (
        <ThemedText style={[styles.cellText, styles.priceText]} numberOfLines={1} ellipsizeMode="tail">
  ...
  },
  {
    ...
    accessor: (airbrushing: Airbrushing) => (
      <ThemedText style={styles.cellText} numberOfLines={1} ellipsizeMode="tail">
        {airbrushing.startDate ? formatDate(airbrushing.startDate) : "-"}
      </ThemedText>
    ),
  },
  {
    ...
    accessor: (airbrushing: Airbrushing) => (
      <ThemedText style={styles.cellText} numberOfLines={1} ellipsizeMode="tail">
        {airbrushing.finishDate ? formatDate(airbrushing.finishDate) : "-"}
      </ThemedText>
    ),
  },
  {
    ...
    accessor: (airbrushing: Airbrushing) => (
      <ThemedText style={styles.cellText} numberOfLines={1} ellipsizeMode="tail">
        {airbrushing.createdAt ? formatDate(airbrushing.createdAt) : "-"}
      </ThemedText>
    ),
  },
];

export const AirbrushingTable = React.memo<AirbrushingTableProps>(
  ({
    airbrushings,
    onAirbrushingPress,
    onAirbrushingEdit,
    ...
    loadingMore = false,
    showSelection = false,
    selectedAirbrushings = new Set(),
    onSelectionChange,
    sortConfigs = [],
    onSort,
    visibleColumnKeys = ["task", "status", "price"],
    ...
    const handleSelectAll = useCallback(() => {
      ...
      const allSelected = airbrushings.every((airbrushing) => selectedAirbrushings.has(airbrushing.id));
      if (allSelected) {
        onSelectionChange(new Set());
      } else {
        onSelectionChange(new Set(airbrushings.map((airbrushing) => airbrushing.id)));
      }
    }, [airbrushings, selectedAirbrushings, onSelectionChange]);

    const handleSelectAirbrushing = useCallback(
      (airbrushingId: string) => {
        if (!onSelectionChange) return;

        const newSelection = new Set(selectedAirbrushings);
        if (newSelection.has(airbrushingId)) {
          newSelection.delete(airbrushingId);
        } else {
          newSelection.add(airbrushingId);
        }
        onSelectionChange(newSelection);
      },
      [selectedAirbrushings, onSelectionChange],
    );
    ...
    const renderColumnValue = useCallback((airbrushing: Airbrushing, column: TableColumn) => {
      return column.accessor(airbrushing);
    }, []);
    ...
    const handleViewableItemsChanged = React.useMemo(
      () => ({
        onViewableItemsChanged: ({ viewableItems }: any) => {
          ...
          if (lastViewableIndex !== undefined && lastViewableIndex !== null) {
            const totalItems = airbrushings.length;
      ...
      [onPrefetch, airbrushings.length],
    );
    ...
    const renderHeader = useCallback(
            ...
            <View style={StyleSheet.flatten([styles.headerRow, { width: tableWidth }])}>
              {showSelection && (
                <View style={StyleSheet.flatten([styles.headerCell, styles.checkboxCell])}>
                  <Checkbox
                    checked={airbrushings.length > 0 && airbrushings.every((airbrushing) => selectedAirbrushings.has(airbrushing.id))}
                    onCheckedChange={handleSelectAll}
                    disabled={airbrushings.length === 0}
                  />
                </View>
              )}
      ...
      [colors, isDark, tableWidth, displayColumns, showSelection, selectedAirbrushings, airbrushings.length, sortConfigs, handleSelectAll, handleSort],
    );

    // Row component
    const renderRow = useCallback(
      ({ item, index }: { item: Airbrushing; index: number }) => {
        const isSelected = selectedAirbrushings.has(item.id);
        const isEven = index % 2 === 0;

        if (enableSwipeActions && (onAirbrushingEdit || onAirbrushingDelete)) {
          return (
            <AirbrushingTableRowSwipe
              key={item.id}
              airbrushingId={item.id}
              airbrushingName={(item as any).task?.name || "Airbrushing"}
                  ...
                  >
                    {showSelection && (
                      <View style={StyleSheet.flatten([styles.cell, styles.checkboxCell])}>
                        <Checkbox checked={isSelected} onCheckedChange={() => handleSelectAirbrushing(item.id)} />
                      </View>
                    )}
                    {displayColumns.map((column) => (
                      <View
      ...
      [
        ...
        displayColumns,
        showSelection,
        selectedAirbrushings,
        onAirbrushingPress,
        handleSelectAirbrushing,
        renderColumnValue,
        enableSwipeActions,
        onAirbrushingEdit,
        onAirbrushingDelete,
    ...
    const renderEmpty = useCallback(
      () => (
        <View style={styles.emptyContainer}>
          <Icon name="paint" size="xl" variant="muted" />
          <ThemedText style={styles.emptyTitle}>Nenhum airbrushing encontrado</ThemedText>
          <ThemedText style={styles.emptySubtitle}>Tente ajustar os filtros ou adicionar novos airbrushings</ThemedText>
        </View>
      ),
      [],
    );
    ...
    if (loading) {
      return (
        <View style={styles.loadingContainer}>
          ...
          <ThemedText style={styles.loadingText}>Carregando airbrushings...</ThemedText>
        </View>
      );
    }
      ...
      <View style={styles.wrapper}>
        <Pressable style={StyleSheet.flatten([styles.container, { backgroundColor: colors.background, borderColor: colors.border }])} onPress={handleContainerPress}>
          ...
          <FlatList
            ...
            keyExtractor={(airbrushing) => airbrushing.id}
...
AirbrushingTable.displayName = "AirbrushingTable";