...
const ServiceOrderTableRowSwipeComponent = ({
  ...
  const isThisRowActive = activeRowId === serviceOrderId;
  ...
  useEffect(() => {
    return () => {
      ...
      if (activeRowId === serviceOrderId) {
        setActiveRowId(null);
      }
    };
  }, [activeRowId, serviceOrderId, setActiveRowId]);

  const handleDeletePress = useCallback(() => {
    Alert.alert("Confirmar exclusão", `Tem certeza que deseja excluir "${serviceOrderDescription}"?`, [
      {
        text: "Cancelar",
        style: "cancel",
      ...
      },
      {
        ...
        onPress: () => {
          swipeableRef.current?.close();
          setTimeout(() => onDelete?.(serviceOrderId), 300);
        },
      },
    ]);
  }, [serviceOrderId, serviceOrderDescription, onDelete]);
  ...
  const handleWillOpen = useCallback(
    (direction: "left" | "right") => {
      ...
      if (activeRowId && activeRowId !== serviceOrderId) {
        closeActiveRow();
        closeOpenRow(); // Also close legacy rows
      }
    },
    [activeRowId, serviceOrderId, closeActiveRow, closeOpenRow],
  );

  const handleOpen = useCallback(
    (direction: "left" | "right", swipeable: Swipeable) => {
      setActiveRowId(serviceOrderId);
    ...
    [serviceOrderId, setActiveRowId, setOpenRow],
  );
  ...
  if (!children || (typeof children !== "object" && typeof children !== "string" && typeof children !== "number" && typeof children !== "function")) {
    console.warn("ServiceOrderTableRowSwipe: children prop is invalid or undefined:", typeof children);
