...
export const externalOperationItemSelectionSchema = z.object({
  ...
  // Price is required only if type is CHARGEABLE - validated at form level
  price: z.number().min(0, "Preço não pode ser negativo").nullable().optional(),
});

export const externalOperationStage2Schema = z.object({
  items: z.array(externalOperationItemSelectionSchema).min(1, "Selecione pelo menos um item").max(100, "Limite máximo de 100 itens por retirada"),
});
...
// Service item schema (CHARGEABLE billing — ad-hoc services)
export const externalOperationServiceItemCreateSchema = z.object({
  // id is accepted on update payloads (preserve/replace existing rows)
...
export const externalOperationCreateSchema = z
  .object({
    // Optional — the operation must reference a customer OR a withdrawer name
    // (validated in superRefine below; CHARGEABLE always requires a customer).
    ...
    paymentConfig: z.record(z.any()).nullable().optional(),
    services: z.array(externalOperationServiceItemCreateSchema).optional(),
  })
  // Conditional validation mirroring the API schema (CHARGEABLE billing rules)
  .superRefine((data, ctx) => {
    const items = data.items ?? [];
    const services = data.services ?? [];

    // Responsible reference: customer OR withdrawer name must be informed
    if (!data.withdrawerName?.trim() && !data.customerId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Informe o responsável ou selecione um cliente",
        path: ["withdrawerName"],
      });
    }

    // CHARGEABLE operations always require a customer (billing target)
    if (data.type === EXTERNAL_OPERATION_TYPE.CHARGEABLE && !data.customerId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export const externalOperationUpdateSchema = z.object({
  withdrawerName: z
    .string()
  ...
  invoiceIds: z.array(z.string().uuid("NFe inválida")).optional(),
  receiptIds: z.array(z.string().uuid("Recibo inválido")).optional(),
  // Items (delete-recreate on update — mirrors API update schema)
  items: z
    .array(
      z.object({
        itemId: z.string().uuid("Item inválido"),
  ...
  // (cross-field rules against the persisted entity are enforced server-side)
  if (data.type !== undefined && data.type !== EXTERNAL_OPERATION_TYPE.CHARGEABLE && (data.services?.length ?? 0) > 0) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "Serviços só podem ser adicionados em retiradas do tipo Cobrável",
      path: ["services"],
