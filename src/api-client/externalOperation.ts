...
export class ExternalOperationService {
  ...
  /**
   * Manually (re)triggers the billing pipeline (NFS-e + boletos) for a CHARGED
   * withdrawal with billing configured and no active invoice.
   * POST /external-operations/:id/generate-billing (ADMIN, FINANCIAL).
   */
  async generateBilling(id: string, queryParams?: ExternalOperationQueryFormData): Promise<ExternalOperationUpdateResponse> {
    const response = await apiClient.post<ExternalOperationUpdateResponse>(`${this.basePath}/${id}/generate-billing`, undefined, {
      params: queryParams,
    });
    return response.data;
  }
  ...
  async batchUpdateExternalOperations(
    ...
    const response = await apiClient.put<ExternalOperationBatchUpdateResponse<ExternalOperation>>(`${this.basePath}/batch`, data, {
      params: queryParams,
    });
    return response.data;
  }
...
// Billing convenience export
export const generateExternalOperationBilling = (id: string, queryParams?: ExternalOperationQueryFormData) =>
  externalOperationService.generateBilling(id, queryParams);
