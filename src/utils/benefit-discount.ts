...
//
// Espelho de api/src/utils/benefit-discount.ts e web/src/utils/benefit-discount.ts.
// É a única fonte de verdade para o cálculo do desconto do colaborador (parte do
// colaborador) e do custo da empresa (parte da empresa) de uma adesão de
// benefício (UserBenefit). Consumida pelas telas de Adesões (preview do form e
// cards de detalhe).
//
...
export interface BenefitShareSplit {
   ...
   * chamador deve sinalizar a pendência ao usuário.
   */
 ...
 */
export function isSalaryUnknownForShare(rule: BenefitShareRule, baseSalary?: number | null): boolean {
 ...
 */
export function calculateEmployeeShare(rule: BenefitShareRule, baseSalary?: number | null): number {
 ...
 */
export function calculateBenefitSplit(rule: BenefitShareRule, baseSalary?: number | null): BenefitShareSplit {
  const monthlyValue = Math.max(rule.monthlyValue ?? 0, 0);
