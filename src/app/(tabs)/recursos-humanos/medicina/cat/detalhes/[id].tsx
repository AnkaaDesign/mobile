...
} from "@/components/human-resources/work-accident/detail";
...
export default function WorkAccidentDetailScreen() {
  ...
  return (
    <DetailScreen
      ...
      editRoute={(r: any) =>
        mobileRoute(routes.humanResources.occupationalHealth.workAccidents.edit(r.id))
      }
      deleteAction={{
        mutation: deleteMutation,
        confirmText:
          "Tem certeza que deseja excluir esta CAT? Esta ação não pode ser desfeita.",
        successRoute: mobileRoute(routes.humanResources.occupationalHealth.workAccidents.root),
      }}
      notFoundFallback={mobileRoute(routes.humanResources.occupationalHealth.workAccidents.root)}
    >
      {(report: any) => (
