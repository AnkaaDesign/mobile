...
import { usePaintGroundMutations, usePaints } from '../../../../hooks';
import { paintGroundCreateSchema } from '../../../../schemas';
import type { PaintGroundCreateFormData } from '../../../../schemas';
...
export default function CreatePaintGroundScreen() {
  const { colors } = useTheme();
  const { user } = useAuth();
  const { create } = usePaintGroundMutations();
  ...
  // Form setup
  const form = useForm<PaintGroundCreateFormData>({
    resolver: zodResolver(paintGroundCreateSchema),
    defaultValues: {
      paintId: "",
      groundPaintId: "",
    },
  ...
  const onSubmit = async (data: PaintGroundCreateFormData) => {
    if (!canCreate) {
    ...
    try {
      ...
      showToast("Base de tinta criada com sucesso", "success");
      router.back();
    } catch (error) {
      showToast("Erro ao criar base de tinta", "error");
    } finally {
    ...
    return (
      <View style={styles.centerContainer}>
        <ThemedText style={styles.errorText}>
          Você não tem permissão para criar bases de tinta
        </ThemedText>
  ...
  if (isLoadingPaints) {
    return (
      <View style={styles.centerContainer}>
        ...
        <ThemedText style={styles.loadingText}>Carregando tintas...</ThemedText>
      </View>
    );
  }
      ...
      >
        ...
        <Card style={styles.headerCard}>
          <View style={styles.headerContent}>
            ...
            <View style={styles.headerText}>
              <ThemedText style={styles.headerTitle}>Nova Base de Tinta</ThemedText>
              <ThemedText style={styles.headerSubtitle}>
                Defina qual tinta base é necessária para uma tinta específica
              </ThemedText>
            </View>
          </View>
        </Card>
        ...
        <Card style={styles.formCard}>
          <View style={styles.formSection}>
            <View style={styles.sectionHeader}>
              ...
              <ThemedText style={styles.sectionTitle}>Tinta Principal</ThemedText>
            </View>
            <ThemedText style={styles.sectionDescription}>
              Selecione a tinta que precisa de uma base específica
            </ThemedText>
        ...
        <Card style={styles.formCard}>
          <View style={styles.formSection}>
            <View style={styles.sectionHeader}>
              <IconPalette size={20} color={colors.primary} />
              <ThemedText style={styles.sectionTitle}>Tinta Base</ThemedText>
            </View>
            <ThemedText style={styles.sectionDescription}>
              Selecione a tinta base necessária para a tinta principal
            </ThemedText>
        ...
        {/* Preview */}
        {selectedPaint && selectedGroundPaint && (
          <Card style={styles.previewCard}>
            <View style={styles.previewHeader}>
              <IconCheck size={20} color={colors.success} />
              <ThemedText style={styles.previewTitle}>Pré-visualização</ThemedText>
            </View>
        ...
        <View style={styles.actionButtons}>
          ...
          <Button
            ...
            disabled={!isValid || isSubmitting}
          >
            {isSubmitting ? "Criando..." : "Criar Base de Tinta"}
          </Button>
        </View>
      </ScrollView>
    </>
