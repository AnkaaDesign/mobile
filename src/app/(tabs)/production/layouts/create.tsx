...
export default function LayoutCreateScreen() {
  ...
  const form = useForm<LayoutCreateFormData>({
    resolver: zodResolver(layoutCreateSchema),
    defaultValues: {
      height: 2.4,
      sections: [
        {
  ...
  const { fields: sections, append: addSection, remove: removeSection, update: updateSection } = useFieldArray({
    control: form.control,
    name: "sections",
  });

  const onSubmit = async (data: LayoutCreateFormData) => {
