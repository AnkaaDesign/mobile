...
export default function PersonalBonusSimulationScreen() {
  ...
  // Calculate average tasks per user (company-wide average)
  const averageTasksPerUser = useMemo(() => {
    if (eligibleUsersCount === 0) return 0;
    return totalWeightedTasks / eligibleUsersCount;
  }, [totalWeightedTasks, eligibleUsersCount]);
  ...
  // Current user's actual bonus (with current position, performance, and real average)
  const currentBonus = useMemo(() => {
    if (!currentUser?.position?.name) return 0;
    const currentPerformance = currentUser.performanceLevel || 3;
    // Use the actual current average, not the simulated one
    return calculateBonusForPosition(currentUser.position.name, currentPerformance, averageTasksPerUser);
  }, [currentUser, averageTasksPerUser]);
  ...
  return (
    <ThemedView style={[styles.container, { backgroundColor: colors.background, paddingBottom: insets.bottom }]}>
      <ScrollView style={styles.scrollView} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh} colors={[colors.primary]} tintColor={colors.primary} />}>
        ...
        {/* Current Bonus Card */}
        <Card style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <ThemedText style={styles.sectionTitle}>Seu Bônus Atual</ThemedText>
          <ThemedText style={[styles.currentBonusAmount, { color: colors.success }]}>
            {formatCurrency(currentBonus)}
          </ThemedText>
          <View style={styles.currentBonusDetails}>
            <View style={styles.detailRow}>
              <ThemedText style={[styles.detailLabel, { color: colors.mutedForeground }]}>Cargo:</ThemedText>
              <ThemedText style={styles.detailValue}>{currentUser.position?.name || "-"}</ThemedText>
            </View>
            <View style={styles.detailRow}>
              <ThemedText style={[styles.detailLabel, { color: colors.mutedForeground }]}>Desempenho:</ThemedText>
              <ThemedText style={styles.detailValue}>Nível {currentUser.performanceLevel || 3}</ThemedText>
            </View>
            <View style={styles.detailRow}>
              <ThemedText style={[styles.detailLabel, { color: colors.mutedForeground }]}>Tarefas Ponderadas:</ThemedText>
              <ThemedText style={styles.detailValue}>{totalWeightedTasks.toFixed(1)}</ThemedText>
            </View>
            <View style={styles.detailRow}>
            ...
            <View style={styles.detailRow}>
              <ThemedText style={[styles.detailLabel, { color: colors.mutedForeground }]}>Média de Tarefas:</ThemedText>
              <ThemedText style={styles.detailValue}>{averageTasksPerUser.toFixed(2)}</ThemedText>
            </View>
          </View>
        </Card>
        ...
        {/* Task Details Card */}
        <Card style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <ThemedText style={styles.sectionTitle}>Detalhes das Tarefas</ThemedText>
          <View style={styles.currentBonusDetails}>
            <View style={styles.detailRow}>
              <Badge variant="destructive" size="sm">
                Comissão Suspensa
              </Badge>
              <ThemedText style={styles.detailValue}>{tasksByCommission.suspendedCommission}</ThemedText>
            </View>
            <View style={styles.detailRow}>
              <Badge variant="warning" size="sm">
                Não Comissionadas
              </Badge>
              <ThemedText style={styles.detailValue}>{tasksByCommission.noCommission}</ThemedText>
            </View>
            <View style={styles.detailRow}>
              <Badge variant="primary" size="sm">
                Parcialmente Comissionadas
              </Badge>
              <ThemedText style={styles.detailValue}>{tasksByCommission.partialCommission}</ThemedText>
            </View>
            <View style={styles.detailRow}>
              <Badge variant="success" size="sm">
                Totalmente Comissionadas
              </Badge>
              <ThemedText style={styles.detailValue}>{tasksByCommission.fullCommission}</ThemedText>
            </View>
          </View>
        </Card>
        ...
        <Card style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          {/* Position Selector */}
          <View style={styles.inputGroup}>
            <ThemedText style={styles.inputLabel}>Cargos</ThemedText>
          ...
          <View style={styles.inputGroup}>
            <ThemedText style={styles.inputLabel}>Desempenho</ThemedText>
        ...
        <Card style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <ThemedText style={styles.sectionTitle}>Bônus Simulado</ThemedText>
          ...
          {calculatedBonus !== currentBonus && (
            <View style={[styles.differenceCard, { backgroundColor: colors.muted, borderColor: colors.border }]}>
              <ThemedText style={[styles.differenceLabel, { color: colors.mutedForeground }]}>
                Diferença do seu bônus atual:
              </ThemedText>
              <ThemedText
                style={[
                  styles.differenceValue,
                  { color: calculatedBonus > currentBonus ? colors.success : colors.destructive },
                ]}
              >
                {calculatedBonus > currentBonus ? "+" : ""}
                {formatCurrency(calculatedBonus - currentBonus)}
              </ThemedText>
            </View>
          )}
        </Card>
