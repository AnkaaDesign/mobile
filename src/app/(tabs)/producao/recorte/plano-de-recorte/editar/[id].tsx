...
import { cutUpdateSchema, type CutUpdateFormData } from "@/schemas";
import {
  CUT_STATUS,
  CUT_TYPE,
  CUT_ORIGIN,
...
export default function EditCuttingPlanScreen() {
  ...
  const onSubmit = async (data: CutUpdateFormData) => {
    if (!canEdit) {
      Alert.alert("Erro", "Você não tem permissão para editar planos de recorte");
      return;
    }

    if (!id) {
      Alert.alert("Erro", "ID do plano de recorte não encontrado");
      return;
    }
    ...
    try {
      ...
      Alert.alert("Sucesso", "Plano de recorte atualizado com sucesso!", [
        {
          text: "OK",
          onPress: () => router.back(),
        },
      ...
      Alert.alert("Erro", error?.message || "Não foi possível atualizar o plano de recorte. Tente novamente.");
    } finally {
    ...
    return (
      <>
        ...
        <ErrorScreen
          ...
          detail="Você não tem permissão para editar planos de recorte. É necessário privilégio de Produção, Almoxarifado ou Administrador."
        />
      </>
    );
  }
  ...
  if (isLoading) {
    return (
      <>
        ...
        />
        <LoadingScreen message="Carregando dados do plano de recorte..." />
      </>
    );
  }
  ...
  if (error || !cut) {
    return (
      <>
        ...
        />
        <ErrorScreen
          message="Erro ao carregar plano de recorte"
          detail={error?.message || "Plano de recorte não encontrado"}
        />
      </>
    );
  }
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        >
          ...
          <Card style={{ padding: spacing.md, marginBottom: spacing.md }}>
            ...
            />
            <ThemedText style={{ fontSize: 12, color: colors.muted, marginTop: spacing.xs }}>
              Altere o status do plano de recorte conforme o progresso
            </ThemedText>
          </Card>
          ...
          {cut.parentCutId && (
            <Card style={{ padding: spacing.md, marginBottom: spacing.md }}>
              ...
              />
              <ThemedText style={{ fontSize: 12, color: colors.muted, marginTop: spacing.xs }}>
                Este é um retrabalho. O corte pai não pode ser alterado.
              </ThemedText>
            </Card>
          )}
