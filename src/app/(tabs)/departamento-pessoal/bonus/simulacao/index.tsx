...
export default function BonusSimulationScreen() {
  ...
  const { data: usersData, isLoading: usersLoading, refetch: refetchUsers } = useUsers({
    where: {
      // Bonus eligibility = CLT confirmado (ACTIVE) — mirrors api isBonifiable.
            ...
            {simulatedUsers.map((user) => (
              ...
              >
                <View style={styles.userHeader}>
                  <ThemedText style={styles.userName} numberOfLines={2}>{user.name}</ThemedText>
                  <ThemedText style={[styles.bonusAmount, { color: colors.success }]} numberOfLines={1}>{formatCurrency(user.bonusAmount)}</ThemedText>
                </View>
