...
import { PPE_DELIVERY_STATUS, PPE_DELIVERY_STATUS_ORDER, PPE_TYPE, SECTOR_PRIVILEGES, CONTRACT_STATUS, routes } from "@/constants";
...
function CreateHRPPEDeliveryScreenInner() {
  ...
  const handleSubmit = async (data: PpeDeliveryCreateFormData) => {
    try {
      ...
      if (newId) {
        nav.replace(mobileRoute(routes.personnelDepartment.ppe.deliveries.details(newId)));
      } else {
        goBack();
      }
    } catch {
