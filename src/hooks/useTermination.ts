...
import { terminationKeys, userKeys, changeLogKeys } from "./queryKeys";
...
const baseHooks = createEntityHooks<
  ...
  TerminationDeleteResponse,
  TerminationBatchCreateFormData,
  TerminationBatchCreateResponse<Termination>,
  TerminationBatchUpdateFormData,
  TerminationBatchUpdateResponse<Termination>,
  TerminationBatchDeleteFormData,
  TerminationBatchDeleteResponse
>({
...
export function useTerminationDocumentUpload() {
  const invalidate = useInvalidateTerminations();
  return useMutation({
    mutationFn: ({ id, data, file }: { id: string; data: TerminationDocumentUploadFormData; file: any }) => uploadTerminationDocument(id, data, file),
    onSuccess: invalidate,
  });
}
