...
function stripAutoGeneratedFields(value: any): any {
  ...
  if (value && typeof value === 'object' && !(value instanceof Date)) {
    ...
    for (const key of Object.keys(value)) {
      ...
      // Skip file-like objects
      if (value[key] && typeof value[key] === 'object' && ('uploaded' in value[key] || 'uri' in value[key])) {
        continue;
      }
 ...
 * CRITICAL: This function determines if a field has changed.
 */
function deepCompare(value1: any, value2: any): boolean {
  ...
  // Handle arrays - Order matters for arrays like services
...
export function useEditForm<TFieldValues extends FieldValues = FieldValues, TContext = unknown, TApiData = unknown>({
  ...
  const getChangedFields = useCallback(() => {
    ...
    allKeys.forEach((key) => {
      ...
      // Special handling for quote - check nested items
      if (key === 'quote') {
        const currentQuote = currentValue as any;
        const originalQuote = originalValue as any;

        if (currentQuote && originalQuote) {
          // Strip auto-generated ids and filter empty services from quote.services
