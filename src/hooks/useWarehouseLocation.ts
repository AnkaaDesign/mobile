...
import { warehouseLocationKeys, itemKeys, changeLogKeys } from "./queryKeys";
...
export function useCreateWarehouseLocation() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: warehouseLocationKeys.statistics() });
...
export function useUpdateWarehouseLocation(id: string, queryParams?: WarehouseLocationQueryFormData) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: warehouseLocationKeys.all });
      queryClient.invalidateQueries({ queryKey: warehouseLocationKeys.statistics() });
      queryClient.invalidateQueries({ queryKey: itemKeys.byWarehouseLocation(id) });
      queryClient.invalidateQueries({ queryKey: changeLogKeys.all });
    },
  });
}
    ...
    onSuccess: (response) => {
      ...
      if (response.data?.success) {
        response.data.success.forEach((location) => {
          ...
          queryClient.invalidateQueries({ queryKey: itemKeys.byWarehouseLocation(location.id) });
        });
      }
...
export function useWarehouseLocationBatchMutations() {
  const batchCreateMutation = useBatchCreateWarehouseLocations();
  const batchUpdateMutation = useBatchUpdateWarehouseLocations();
