...
>({
  ...
  relatedQueryKeys: [userKeys, changeLogKeys], // Admission create/advance updates the user
});
...
export function useAdmissionDocumentUpload() {
  const invalidate = useInvalidateAdmissions();
  return useMutation({
    mutationFn: ({ id, data, file }: { id: string; data: AdmissionDocumentUploadFormData; file: any }) => uploadAdmissionDocument(id, data, file),
    onSuccess: invalidate,
  });
}
...
export function useAdmissionDocumentUploadByUser() {
  const invalidate = useInvalidateAdmissions();
  return useMutation({
    mutationFn: ({ userId, data, file }: { userId: string; data: AdmissionDocumentUploadFormData; file: any }) => uploadAdmissionDocumentByUser(userId, data, file),
    onSuccess: invalidate,
  });
}
