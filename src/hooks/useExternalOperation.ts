...
import { externalOperationKeys, externalOperationItemKeys, itemKeys, activityKeys, changeLogKeys } from "./queryKeys";
 ...
 * CHARGEABLE operation with billing configured and no active invoice.
 */
export function useGenerateExternalOperationBilling() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ id, queryParams }: { id: string; queryParams?: any }) => generateExternalOperationBilling(id, queryParams),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: externalOperationKeys.all });
      queryClient.invalidateQueries({ queryKey: externalOperationKeys.detail(id) });
      queryClient.invalidateQueries({ queryKey: changeLogKeys.all });
