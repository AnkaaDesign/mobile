...
export interface ExternalOperationFormStateData {
  // Stage navigation
  stage: ExternalOperationFormStage;
  formTouched: boolean;
...
interface UseExternalOperationFormStateOptions {
  storageKey?: string;
  ...
  /**
   * Enforce customer selection for CHARGEABLE operations (create flow —
   * mirrors the API create rule). The edit form leaves this off so legacy
   * CHARGEABLE operations without a customer remain editable.
   */
  requireCustomerOnChargeable?: boolean;
  initialData?: Partial<ExternalOperationFormStateData>;
  autoSave?: boolean;
  onStateChange?: (state: ExternalOperationFormStateData) => void;
}

const DEFAULT_STATE: ExternalOperationFormStateData = {
  stage: 1,
  formTouched: false,
  selectedItems: [],
  quantities: {},
  ...
  withdrawerName: "",
  customerId: null,
  type: EXTERNAL_OPERATION_TYPE.RETURNABLE,
  notes: "",
  nfeId: null,
  receiptId: null,
...
export function useExternalOperationFormState(
  options: UseExternalOperationFormStateOptions = {},
) {
  const {
    storageKey = "@external_operation_form",
    defaultQuantity = 1,
    defaultPrice = 0,
    preserveQuantitiesOnDeselect = false,
    defaultPageSize = 40,
    validateOnStageChange = true,
    requireCustomerOnChargeable = false,
    initialData,
    autoSave = true,
    onStateChange,
  } = options;
  ...
  const [state, setState] = useState<ExternalOperationFormStateData>(() => ({
    ...DEFAULT_STATE,
  ...
  useEffect(() => {
    const loadState = async () => {
      try {
        ...
        if (stored) {
          const parsed = JSON.parse(stored) as ExternalOperationFormStateData;
          setState((prev) => ({
            ...prev,
            ...parsed,
        ...
        console.error("[ExternalOperationForm] Failed to load state:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const saveState = async () => {
      ...
      try {
        ...
        console.error("[ExternalOperationForm] Failed to save state:", error);
      } finally {
        setIsSaving(false);
      }
    };
  ...
  const validation = useMemo((): ExternalOperationFormValidationState => {
    ...
    // withdrawerName is optional — the operation needs a customer OR a
    // responsible name. CHARGEABLE operations always require a customer
    // (billing target), mirroring the API create rules.
    let stage1Valid = true;
    const trimmedName = withdrawerName?.trim() || "";
    if (trimmedName && trimmedName.length < 2) {
      errors.withdrawerName = "Nome do responsável deve ter pelo menos 2 caracteres";
      stage1Valid = false;
    }
    if (!trimmedName && !customerId) {
      errors.withdrawerName = "Informe o responsável ou selecione um cliente";
      stage1Valid = false;
    }
    if (requireCustomerOnChargeable && type === EXTERNAL_OPERATION_TYPE.CHARGEABLE && !customerId) {
      errors.customerId = "Selecione o cliente para operações cobráveis";
      stage1Valid = false;
    }
  ...
  }, [withdrawerName, customerId, type, requireCustomerOnChargeable, selectedItems, quantities, prices]);

  // Update functions
  const updateState = useCallback(
    (updates: Partial<ExternalOperationFormStateData>) => {
  ...
  const clearAllSelections = useCallback(() => {
    const resetData: Partial<ExternalOperationFormStateData> = {
      selectedItems: [],
    };
  ...
  const getFormData = useCallback(() => {
    return {
      // withdrawerName is optional — an operation needs a customer OR a name.
      withdrawerName: withdrawerName.trim() || undefined,
      ...
      // Item payload must match externalOperationCreateSchema:
      ...
      items: getSelectedItemsWithData().map((item) => ({
        ...
        price: type === EXTERNAL_OPERATION_TYPE.CHARGEABLE ? item.price || 0 : undefined,
      })),
    };
  }, [withdrawerName, customerId, type, notes, getSelectedItemsWithData]);
  ...
  const resetForm = useCallback(async () => {
    ...
    try {
      ...
      console.error("[ExternalOperationForm] Failed to clear storage:", error);
    }
  ...
  }, [withdrawerName, customerId, type, notes, selectedItems.size, nfeId, receiptId]);
  ...
  }, [withdrawerName, customerId, type, notes, nfeId, receiptId, selectedItems.size]);
 ...
 *       >
 *         <Picker.Item label="Retornável" value={EXTERNAL_OPERATION_TYPE.RETURNABLE} />
 *         <Picker.Item label="Cobrável" value={EXTERNAL_OPERATION_TYPE.CHARGEABLE} />
 *         <Picker.Item label="Cortesia" value={EXTERNAL_OPERATION_TYPE.COMPLIMENTARY} />
 *       </Picker>
 *       <TextInput
 *         value={notes}
 ...
 *                 />
 *                 {type === EXTERNAL_OPERATION_TYPE.CHARGEABLE && (
 *                   <TextInput
 *                     keyboardType="numeric"
 ...
 * ```tsx
 * const hook = useExternalOperationFormState({
 *   storageKey: '@external_operation_draft_123',
 *   defaultQuantity: 1,
 *   defaultPrice: 0,
 ...
 * ```tsx
 * const { formProgress, stageHasData, hasFormData, resetForm } = useExternalOperationFormState();
 *
 * // Show progress bar
 ...
 * ```tsx
 * const { nfeId, receiptId, updateNfeId, updateReceiptId } = useExternalOperationFormState();
 *
 ...
 * ```tsx
 * const { saveState, loadState, resetForm } = useExternalOperationFormState({
