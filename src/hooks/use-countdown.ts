 ...
 *
 * @param deadline - Target date/time for countdown
 * @returns Countdown information including time remaining and formatted string
 *
 * @example
 * ```tsx
 * const countdown = useCountdown(orderDeadline);
 *
 * <Text style={countdown.isOverdue ? styles.overdue : styles.normal}>
 *   {countdown.isOverdue ? 'Atrasado: ' : 'Tempo restante: '}
 *   {countdown.formatted}
 * </Text>
 * ```
 */
...
/**
 * Get a human-readable string for the countdown (Portuguese)
 *
 * @param countdown - Countdown result from useCountdown
 * @returns Formatted string in Portuguese
 *
 * @example
 * ```tsx
 * const countdown = useCountdown(deadline);
 * const display = getCountdownDisplay(countdown);
 * // "2d 5h" or "30m atrasado"
 * ```
 */
