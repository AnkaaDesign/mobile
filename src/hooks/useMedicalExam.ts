...
import type { MedicalExamGetManyFormData, MedicalExamCreateFormData, MedicalExamUpdateFormData, MedicalExamCompleteFormData, MedicalExamBatchCreateFormData, MedicalExamBatchUpdateFormData, MedicalExamBatchDeleteFormData } from "@/schemas";
import type {
  MedicalExam,
...
import { medicalExamKeys, changeLogKeys } from "./queryKeys";
...
export function useUploadMedicalExamDocument() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, file, include }: { id: string; file: any; include?: any }) => uploadMedicalExamDocument(id, file, include ? { include } : undefined),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: medicalExamKeys.all });
      queryClient.invalidateQueries({ queryKey: changeLogKeys.all });
    },
