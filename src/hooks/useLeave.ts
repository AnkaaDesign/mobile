...
import { getLeaves, getLeaveById, createLeave, updateLeave, deleteLeave, finishLeave, getLeavePayrollSplit, uploadLeaveFiles, batchCreateLeaves, batchUpdateLeaves, batchDeleteLeaves } from "@/api-client";
import type { LeaveGetManyFormData, LeaveCreateFormData, LeaveUpdateFormData, LeaveBatchCreateFormData, LeaveBatchUpdateFormData, LeaveBatchDeleteFormData } from "@/schemas";
import type {
  Leave,
...
import { leaveKeys, changeLogKeys } from "./queryKeys";
...
export function useUploadLeaveFiles() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, files, include }: { id: string; files: any[]; include?: any }) => uploadLeaveFiles(id, files, include ? { include } : undefined),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: leaveKeys.all });
      queryClient.invalidateQueries({ queryKey: changeLogKeys.all });
    },
