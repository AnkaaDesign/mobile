...
import type { UserBenefitGetManyFormData, UserBenefitCreateFormData, UserBenefitUpdateFormData, UserBenefitBatchCreateFormData, UserBenefitBatchUpdateFormData, UserBenefitBatchDeleteFormData } from "@/schemas";
import type {
  UserBenefit,
...
import { userBenefitKeys, benefitKeys, changeLogKeys } from "./queryKeys";
...
export function useUploadUserBenefitDeclaration() {
  const invalidate = useInvalidateUserBenefits();
  return useMutation({
    mutationFn: ({ id, file, include }: { id: string; file: any; include?: any }) => uploadUserBenefitDeclaration(id, file, include ? { include } : undefined),
    onSuccess: invalidate,
  });
}
